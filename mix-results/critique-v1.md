...
- The reveal is legible. A blunt score plus runway estimate is easy to understand, easy to screenshot, and easy to market.
Brutal Feedback:
...
- "Rehire odds" is where this starts smelling fake. Based on what, exactly? If the score depends on labor-market inputs, industry benchmarks, or hiring climate signals, that dependency is UNVERIFIED. If it does not, then "rehire odds" is just decorative fiction.
...
- What is the one action this app gives that a spreadsheet, a calculator, or ChatGPT would not give just as well?
...
- Can one person ship this in 2-4 weeks with AI coding tools? MAYBE — the calculator and UI are easy, but the credibility problem is the real build. Shipping a fake-authority score quickly is trivial; shipping something users trust is not.
- Biggest solo complexity traps: inventing bogus benchmarks, sourcing labor-market data, making percentile claims without a real dataset, building a retention loop for an inherently infrequent use case, and overcomplicating the scoring model to hide that the core value may be thin.
